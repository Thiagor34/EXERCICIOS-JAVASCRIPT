...
function inserirLinha(tabela, titulo, genero) {
    var linha = tabela.insertRow(-1)                //Adicionar uma linha na tabela

    var col1 = linha.insertCell(0)                  // cria colunas na lihnha inserida
    var col2 = linha.insertCell(1)
    var col3 = linha.insertCell(2)

    col1.textContent = titulo                      //joga um conteúdo em cada célula
    col2.textContent = genero
    col3.innerHTML = "<input type='checkbox'>"      //innerHTML rendeniza código
}
...
function recuperarFilmes() {
    ...
    if (localStorage.getItem("filmesTitulo")) {
        ...
        //cria referencia ao elemento tbFilmes
        var tbFilmes = document.getElementById("tbFilmes")
...
//cria referência ao checkbox ckTodos
var ckTodos = document.getElementById("ckTodos")
//executa a função anônima quando houver uma troca de status
ckTodos.addEventListener("change", function(){
    //cria referência a tabela e aos campos input (filhos da tabela)
    var tbFilmes = document.getElementById("tbFilmes")
    var ckExcluir = tbFilmes.getElementsByTagName("input")

    var status = ckTodos.checked        //obtém status de ckTodos

    //percorre os demais checkbox para aplicar o status
